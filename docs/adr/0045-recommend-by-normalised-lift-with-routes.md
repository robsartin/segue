...
**Amendment (2026-09-08, issue #303): the tenth reading, with the age split moved to the default
change, stood the shipped setting.**
...
A dated note was committed and pushed at 21:15 local on 2026-09-08, before the reading the owner
pasted carrying 21:16 and 21:19 timestamps, appended to
`docs/superpowers/specs/2026-09-04-second-reading-rule-design.md`; pull request #304 carries it.
The note moved the split instant and said why, named the observations this amendment may make
against the ninth reading and the ones it may not, and wrote both sentences for the halves and both
outcomes of the rule in advance. The rule is the authority on what would have counted and is not
restated here.

**The split moved; the rule did not.** The instant carried since the fifth reading predates the
default change, and the ninth reading's amendment records that nothing could be read from a new
half that mixed sessions dealt under `lift` with one dealt under `resource-allocation`. The rule
reads whole-population cells only and the halves decide nothing, so the instant was the note's to
set. This reading is split at the instant pull request #292 merged the new default, so the old
half is everything rated before `resource-allocation` shipped and the new half is only what the
deck dealt from it, on the note's stated assumption that both sessions since ran from main after
that merge. The halves therefore compare to no earlier reading's halves, and this amendment makes
no such comparison.

**The reading.** One run of `./gradlew evaluate` with `--rated-since` at the new instant on the
owner's database on 2026-09-08, on the folded harness, quoted whole and unedited; the census taken
before it is on issue #303. Aggregates only, per [ADR 51](0051-what-an-adr-may-quote.md): every
cell is a count, a one-decimal mean or a dash, every label is a column name or a `Scorer`
spelling, and the header names the folds and the instant. `in pool`, `hits` and the four half
cells count entities; `pool` and `negatives` count entity-folds.
...
# top 25 per setting, over 16 setting(s).
scorer               floor   pool  in pool  hits  mean rank  negatives  neg mean rank  old in pool  old hits  new in pool  new hits
raw                      2  17579      450    49       11.8         24           19.0          299        39          151        10
raw                      5   6988      450    51       12.2         24           19.0          299        41          151        10
raw                      8   5059      427    51       12.2         24           19.0          278        41          149        10
raw                     12   4058      404    51       12.2         24           19.0          255        41          149        10
adamic-adar              2  17579      450    56       10.4         14           18.6          299        45          151        11
adamic-adar              5   6988      450    56       10.4         14           18.6          299        45          151        11
adamic-adar              8   5059      427    56       10.4         14           18.6          278        45          149        11
adamic-adar             12   4058      404    56       10.4         14           18.6          255        45          149        11
resource-allocation      2  17579      450    58       11.4         20           14.1          299        45          151        13
resource-allocation      5   6988      450    58       11.4         20           14.1          299        45          151        13
resource-allocation      8   5059      427    58       11.4         20           14.1          278        45          149        13
resource-allocation     12   4058      404    58       11.4         20           14.1          255        45          149        13
lift                     2  17579      450     0          -          5           18.2          299         0          151         0
lift                     5   6988      450     4       14.8         52           13.7          299         2          151         2
lift                     8   5059      427    19       13.3         47           11.9          278        19          149         0
lift                    12   4058      404    28       12.9         29           11.4          255        28          149         0
```
...
**The observations the note allowed.** Against the ninth reading, on the whole-population cells
the unchanged graph makes comparable: the shipped row's `in pool` rose by about an eighth, its
`hits` fell by a few, and its rate fell by under three points. The previously shipped row, `lift`
at the shipped floor, records a few hits in both readings, a rate under a point in each.

**The halves, read as the note directed.** This is the first reading where the new half is only
what the deck dealt from the scorer under judgement, which is the hazard issue #272's amendment
named, now on its own. The shipped row's new-half rate is not above its old-half rate; it is
below it, by some six points. The note wrote this case's sentence in advance, and it is this one:
this reading shows no such lift in the half the deck dealt, and no more than that. No inference
about the scorer is drawn from either half.

### What this does and does not establish

- **It does not establish that the shipped setting is the best one.** It establishes that on this
  reading, by this rule, nothing displaced it, and that the rule applied unchanged, as it has to
  every reading since the second. ADR 65's first consequence is the governing one: no row of that
  table means anything on its own.
- **It does not establish that the deck's dealing is invisible in the harness.** One reading with
  the split in the right place shows one new half that is not above its old half. The note allowed
  the sentence above and nothing stronger, and a later reading with the same split may say
  otherwise.
- **The negatives condition was dropped rather than satisfied**, so nothing here is a finding about
  the negatives column; the cells are quoted above, on the entity-fold scale, and decided nothing.
...
**Amendment (2026-09-13, issue #317): the twelfth reading, the first after a deck session since
the known-list run, stood the shipped setting with its rate at the shipped floor within a point of
the eleventh's.**

Nothing above is withdrawn and no decision above is edited, including the twelve amendments
immediately above this one. No constant changed and no code changed. What changed is the taste
layer: the owner ran the rating deck ([ADR 46](0046-the-rating-deck.md)) on 2026-09-13, the first
change to the ratings since the tenth reading. Between the eleventh reading and that session the
only run against the graph was the promotion expander's first `--known` run
([ADR 66](0066-expand-every-promotion-from-a-dev-tool.md); the run and the census before and after
are on issue #313), which appended corroboration rows and no node and left the census unchanged.
So the eligible population is not the eleventh's, no cell is comparable row for row, and this
amendment sets no cell beside the eleventh's.

**The rule was fixed before the number existed, for the twelfth time.** The rule is the one
described eleven amendments above and it was not changed; "the shipped scorer" is what
`Recommendations.DEFAULT_SCORER` holds, `resource-allocation`, and the shipped floor is unchanged.
A dated note was committed and pushed at 11:33 local on 2026-09-13, before the reading the owner
pasted carrying 12:08 and 12:11 timestamps, appended to
`docs/superpowers/specs/2026-09-04-second-reading-rule-design.md`; pull request #318 carries it.
The note kept the tenth reading's instant, named the observations this amendment may make against
the eleventh — rates, not cells — and wrote three sentences in advance for the cases it foresaw.
The rule is the authority on what would have counted and is not restated here.

**The reading.** One run of `./gradlew evaluate` with `--rated-since` on the owner's database on
2026-09-13, on the folded harness, quoted whole and unedited, taken directly after a census with
the known list that is on issue #317. Aggregates only, per [ADR 51](0051-what-an-adr-may-quote.md):
every cell is a count, a one-decimal mean or a dash, every label is a column name or a `Scorer`
spelling, and the header names the folds and the instant. `in pool`, `hits` and the four half cells
count entities; `pool` and `negatives` count entity-folds.

```
# segue recommender evaluation — aggregates only: no labels, no ids, no notes, no ratings (ADR 51, ADR 63, ADR 65).
# held out every 5 of 478 eligible entity(ies), in 5 fold(s): 478 held out over all folds, at least 382 left on the known-list in each.
# split by rating age at 2026-09-08T02:13:42Z: 299 old (rated before it), 179 new (rated on or after it) — a rating's timestamp is its last write, so a re-rated old promotion counts as new.
# top 25 per setting, over 16 setting(s).
scorer               floor   pool  in pool  hits  mean rank  negatives  neg mean rank  old in pool  old hits  new in pool  new hits
raw                      2  17431      478    52       12.2         22           18.5          299        40          179        12
raw                      5   6787      478    53       12.4         22           18.5          299        40          179        13
raw                      8   4863      456    53       12.4         22           18.5          278        40          178        13
raw                     12   3858      433    53       12.4         22           18.5          255        40          178        13
adamic-adar              2  17431      478    60       10.8         14           19.2          299        45          179        15
adamic-adar              5   6787      478    60       10.8         14           19.2          299        45          179        15
adamic-adar              8   4863      456    60       10.8         14           19.2          278        45          178        15
adamic-adar             12   3858      433    60       10.8         14           19.2          255        45          178        15
resource-allocation      2  17431      478    59       10.8         19           13.9          299        42          179        17
resource-allocation      5   6787      478    59       10.8         19           13.9          299        42          179        17
resource-allocation      8   4863      456    59       10.8         19           13.9          278        42          178        17
resource-allocation     12   3858      433    59       10.8         19           13.9          255        42          178        17
lift                     2  17431      478     0          -          6           19.2          299         0          179         0
lift                     5   6787      478     1       19.0         49           14.0          299         1          179         0
lift                     8   4863      456    21       14.0         46           11.7          278        21          178         0
lift                    12   3858      433    33       13.4         28           11.0          255        33          178         0
```

**What the rule made of it.** Clause 2's void check was run first and did not fire: the smallest
`in pool` cell among the rows any clause compared is far above the bound at which one entity is
worth the whole margin. Clause 3(a) compared every non-shipped scorer's rate at the shipped floor
with `resource-allocation`'s row there. `adamic-adar` is above it, by one entity's worth, about a
fifth of a point; `raw` is over a point below and `lift` some twelve points below. A scorer moves
only on the margin and the dominance condition together, and a fifth of a point is not fifteen, so
no scorer moved and clause 3(b) decided nothing; a near miss stands, and this one is not near.
Clause 4 compared the shipped scorer's rate at every other floor with its rate at the shipped
floor: its `hits` cell is the same at every floor in the grid, so the rate rises only as `in pool`
falls, and at floor twelve it clears under a tenth of the margin. The `negatives` and
`neg mean rank` cells were read for every row and, per clause 5, decided nothing. Outcome: the
shipped setting stands.

**The observations the note allowed.** The header shows the eligible population larger than the
eleventh's, with the old half's `in pool` unchanged and the whole of the growth in the new half.
The shipped setting's whole-population rate at the shipped floor is about half a point below the
eleventh's. Its old-half rate is about a point below the eleventh's and its new-half rate about a
point above; the old half holds the same entities as the eleventh's but, as the header shows, the
folds were drawn over the larger population and the known list they are held out against has
grown, so neither half cell is a re-run of the eleventh's and both are read as rates only. The new
half is the deck's own dealing read back, and no inference about the scorer is drawn from it.

**The first of the note's three sentences applies.** The shipped rate stayed within a few points
of the eleventh's — within a point — so, in the note's words, the session did not move the
reading. Nothing about which half the session's ratings landed in is a finding: the instant fixed
that before the session ran.

### What this does and does not establish

- **It does not establish that the shipped setting is the best one.** It establishes that on this
  reading, by this rule, nothing displaced it, on a population the session enlarged. ADR 65's
  first consequence is the governing one: no row of that table means anything on its own.
- **It does not establish anything about `adamic-adar`.** Its rate at the shipped floor is above
  the shipped scorer's by one entity's worth, and the rule's margin exists so that one entity
  decides nothing. Whether that is a trend is a question for a later reading's note, not this one.
- **The negatives condition was dropped rather than satisfied**, so nothing here is a finding about
  the negatives column; the cells are quoted above, on the entity-fold scale, and decided nothing.
...
- **The rule of issue #245 applies unchanged from here**, with `resource-allocation` as the
  shipped scorer and the floor unchanged.
- **The split instant stays the default change**, and this reading's halves are the tenth's
  instant's halves over a larger population; what a later reading may compare against this one is
  for that reading's note to say.
- **Nothing here is unit-testable.** The harness has its own tests; no behaviour changed, so there
  is no test to write and nothing to see red. The verification is the note that preceded the
  reading, the ruling that applied the rule cell by cell, a byte comparison of the block above
  against the owner's paste, and the full gate over an otherwise unchanged tree — `AdrIndexTest`,
...
**Amendment (2026-09-14, issue #323): the thirteenth reading, the first after the second-hop run,
stood the shipped setting with every cell but the candidate pool identical to the twelfth's.**

Nothing above is withdrawn and no decision above is edited, including the thirteen amendments
immediately above this one. No constant changed and no code changed. What changed is the graph:
the owner ran the promotion expander's first `--second-hop` run
([ADR 66](0066-expand-every-promotion-from-a-dev-tool.md); issue #319) on 2026-09-14, which
visited the unexpanded people and groups beside the known-list acts the graph could not place,
appended a few hundred nodes and edges, and placed one act; the run's block and the known-list
census before and after it are on issue #323. No rating changed. This is the eleventh reading's
order of change, where the graph moved and the ratings held, and the header below shows the
eligible population and its two halves identical in size to the twelfth's, so the folds are the
twelfth's and the cells are comparable row for row, as the note allowed.

**The rule was fixed before the number existed, for the thirteenth time.** The rule is the one
described twelve amendments above and it was not changed; "the shipped scorer" is what
`Recommendations.DEFAULT_SCORER` holds, `resource-allocation`, and the shipped floor is unchanged.
A dated note was committed and pushed at 13:13 local on 2026-09-14, before the reading the owner
pasted carrying 13:29 and 13:31 timestamps, appended to
`docs/superpowers/specs/2026-09-04-second-reading-rule-design.md`; pull request #324 carries it.
The note kept the tenth reading's instant, named the observations this amendment may make against
the twelfth — cells row for row if the eligible count matched, rates otherwise — and wrote its
sentences in advance for the cases it foresaw. The rule is the authority on what would have counted
and is not restated here.

**The reading.** One run of `./gradlew evaluate` with `--rated-since` on the owner's database on
2026-09-14, on the folded harness, quoted whole and unedited, taken after the census with the
known list that is on issue #323. Aggregates only, per [ADR 51](0051-what-an-adr-may-quote.md):
every cell is a count, a one-decimal mean or a dash, every label is a column name or a `Scorer`
spelling, and the header names the folds and the instant. `in pool`, `hits` and the four half cells
count entities; `pool` and `negatives` count entity-folds.
...
# top 25 per setting, over 16 setting(s).
scorer               floor   pool  in pool  hits  mean rank  negatives  neg mean rank  old in pool  old hits  new in pool  new hits
raw                      2  17656      478    52       12.2         22           18.5          299        40          179        12
raw                      5   6863      478    53       12.4         22           18.5          299        40          179        13
raw                      8   4891      456    53       12.4         22           18.5          278        40          178        13
raw                     12   3876      433    53       12.4         22           18.5          255        40          178        13
adamic-adar              2  17656      478    60       10.8         14           19.2          299        45          179        15
adamic-adar              5   6863      478    60       10.8         14           19.2          299        45          179        15
adamic-adar              8   4891      456    60       10.8         14           19.2          278        45          178        15
adamic-adar             12   3876      433    60       10.8         14           19.2          255        45          178        15
resource-allocation      2  17656      478    59       10.8         19           13.9          299        42          179        17
resource-allocation      5   6863      478    59       10.8         19           13.9          299        42          179        17
resource-allocation      8   4891      456    59       10.8         19           13.9          278        42          178        17
resource-allocation     12   3876      433    59       10.8         19           13.9          255        42          178        17
lift                     2  17656      478     0          -          6           19.2          299         0          179         0
lift                     5   6863      478     1       19.0         49           14.0          299         1          179         0
lift                     8   4891      456    21       14.0         46           11.7          278        21          178         0
lift                    12   3876      433    33       13.4         28           11.0          255        33          178         0
```
...
**The observations the note allowed.** Set beside the twelfth's block row for row, every cell of
every row is identical except `pool`, the candidate pool, which is larger at every floor: the run's
nodes and edges put more candidates in the sweep's reach, and neither `in pool` nor `hits` nor any
mean or half cell moved in any row. The shipped setting's whole-population rate, its two halves'
rates, and the `adamic-adar` setting's cells beside it are therefore the twelfth's, cell for cell.
The note's sentence for `in pool` rising while `hits` held does not apply: `in pool` did not rise.
Nothing per entity, and nothing about which acts the run placed: the reading scores rated entities,
and the isolated acts are a different population.

**The first of the note's sentences applies.** The shipped rate stayed within a few points of the
twelfth's — it is the twelfth's — so, in the note's words, the run did not move the reading. That
is the small effect the note expected, at its smallest: the run added neighbourhoods beside acts
that touch nothing rated, and the sweep from the rated entities placed none of what it added in a
top five.

### What this does and does not establish

- **It does not establish that the shipped setting is the best one.** It establishes that on this
  reading, by this rule, nothing displaced it, on the twelfth's population over a larger graph. ADR
  65's first consequence is the governing one: no row of that table means anything on its own.
- **It does not establish anything about `adamic-adar`.** Its cells are the twelfth's, so the gap
  the twelfth's amendment noted — one entity's worth at the shipped floor — is the same observation
  read again over the same rated entities, not a second one; the rule's margin exists so that one
  entity decides nothing. Whether it is a trend is a question for a reading on a changed taste
  layer.
- **It does not establish that the second-hop run did nothing.** The graph moved, as the census on
  issue #323 shows; what this reading measures is the recommender over the rated entities, and the
  run's population was chosen for the acts the graph could not place, which are not among them.
- **The negatives condition was dropped rather than satisfied**, so nothing here is a finding about
  the negatives column; the cells are quoted above, on the entity-fold scale, and decided nothing.
...
- **The rule of issue #245 applies unchanged from here**, with `resource-allocation` as the
  shipped scorer and the floor unchanged.
- **The split instant stays the default change**, and this reading's halves are the twelfth's;
  what a later reading may compare against this one is for that reading's note to say.
- **Nothing here is unit-testable.** The harness has its own tests; no behaviour changed, so there
  is no test to write and nothing to see red. The verification is the note that preceded the
  reading, the ruling that applied the rule cell by cell, a byte comparison of the block above
  against the owner's paste, and the full gate over an otherwise unchanged tree — `AdrIndexTest`,
...
**Amendment (2026-09-15, issue #330): the fourteenth reading, the first with known entities from
outside the concert list, stood the shipped setting with its rate at the shipped floor within a
point of the thirteenth's.**

Nothing above is withdrawn and no decision above is edited, including the fourteen amendments
immediately above this one. No constant changed and no code changed. What changed is both layers,
in this order on 2026-09-15: the promotion expander's first `--known --add` run
([ADR 66](0066-expand-every-promotion-from-a-dev-tool.md); issue #328) over the owner's non-touring
list of authors and comedians, which added entities the graph had never held and expanded them —
the run's block and the census before and after it are on issue #330 — and then a deck session
([ADR 46](0046-the-rating-deck.md)) dealt from that same file, the first change to the ratings since
the twelfth reading and the first dealt from a list other than the concert one. So the eligible
population is not the thirteenth's, no cell is comparable row for row, and this amendment sets no
cell beside the thirteenth's.

**The rule was fixed before the number existed, for the fourteenth time.** The rule is the one
described thirteen amendments above and it was not changed; "the shipped scorer" is what
`Recommendations.DEFAULT_SCORER` holds, `resource-allocation`, and the shipped floor is unchanged.
A dated note was committed and pushed at 12:27 local on 2026-09-15, before the reading, appended to
`docs/superpowers/specs/2026-09-04-second-reading-rule-design.md`; pull request #331 carries it.
The owner ran the harness twice after the note: the first paste lost two cells to the terminal's
width, so he ran it again on the unchanged database, and the second run, carrying 14:55 and 14:58
timestamps, is the one quoted. The note kept the tenth reading's instant, named the observations
this amendment may make against the thirteenth — rates, not cells — and wrote its sentences in
advance for the cases it foresaw. The rule is the authority on what would have counted and is not
restated here.

**The reading.** One run of `./gradlew evaluate` with `--rated-since` on the owner's database on
2026-09-15, on the folded harness, quoted whole and unedited. Aggregates only, per
[ADR 51](0051-what-an-adr-may-quote.md): every cell is a count, a one-decimal mean or a dash, every
label is a column name or a `Scorer` spelling, and the header names the folds and the instant.
`in pool`, `hits` and the four half cells count entities; `pool` and `negatives` count entity-folds.
...
```
# segue recommender evaluation — aggregates only: no labels, no ids, no notes, no ratings (ADR 51, ADR 63, ADR 65).
# held out every 5 of 516 eligible entity(ies), in 5 fold(s): 516 held out over all folds, at least 412 left on the known-list in each.
# split by rating age at 2026-09-08T02:13:42Z: 299 old (rated before it), 217 new (rated on or after it) — a rating's timestamp is its last write, so a re-rated old promotion counts as new.
# top 25 per setting, over 16 setting(s).
scorer               floor   pool  in pool  hits  mean rank  negatives  neg mean rank  old in pool  old hits  new in pool  new hits
raw                      2  17622      512    52       11.9         25           19.4          298        41          214        11
raw                      5   6780      512    52       11.8         25           19.4          298        41          214        11
raw                      8   4809      489    52       11.8         25           19.4          277        41          212        11
raw                     12   3791      465    52       11.8         25           19.4          255        41          210        11
adamic-adar              2  17622      512    60       11.2         20           18.9          298        47          214        13
adamic-adar              5   6780      512    60       11.2         20           18.9          298        47          214        13
adamic-adar              8   4809      489    60       11.2         20           18.9          277        47          212        13
adamic-adar             12   3791      465    60       11.2         20           18.9          255        47          210        13
resource-allocation      2  17622      512    63       11.7         21           13.8          298        45          214        18
resource-allocation      5   6780      512    63       11.7         21           13.8          298        45          214        18
resource-allocation      8   4809      489    63       11.7         21           13.8          277        45          212        18
resource-allocation     12   3791      465    63       11.7         21           13.8          255        45          210        18
lift                     2  17622      512     0          -          7           20.7          298         0          214         0
lift                     5   6780      512     3       22.0         46           14.2          298         3          214         0
lift                     8   4809      489    23       13.5         43           12.0          277        22          212         1
lift                    12   3791      465    33       13.5         33           13.0          255        33          210         0
```

**What the rule made of it.** Clause 2's void check was run first and did not fire: the smallest
`in pool` cell among the rows any clause compared is far above the bound at which one entity is
worth the whole margin. Clause 3(a) compared every non-shipped scorer's rate at the shipped floor
with `resource-allocation`'s row there. Every one is below it: `adamic-adar` by three entities'
worth, about half a point; `raw` by about two points; `lift` by some twelve points. Nothing is
above the shipped scorer, so no scorer moved and clause 3(b) decided nothing; a near miss stands,
and there is none. Clause 4 compared the shipped scorer's rate at every other floor with its rate
at the shipped floor: its `hits` cell is the same at every floor in the grid, so the rate rises
only as `in pool` falls, and at floor twelve it clears under a tenth of the margin. The
`negatives` and `neg mean rank` cells were read for every row and, per clause 5, decided nothing.
Outcome: the shipped setting stands.

**The observations the note allowed.** The header shows the eligible population larger than the
thirteenth's, with the old half the same size and the whole of the growth in the new half. The
shipped setting's whole-population rate at the shipped floor is within a quarter of a point of the
thirteenth's. Its old-half rate is about a point above the thirteenth's and its new-half rate
about a point below; the old half holds the same entities as the thirteenth's but the folds were
drawn over the larger population and the known list they are held out against has grown, so
neither half cell is a re-run of the thirteenth's and both are read as rates only. The new half is
the deck's own dealing read back, and no inference about the scorer is drawn from it. The
`adamic-adar` setting's rate at the shipped floor is now below the shipped scorer's, where at the
twelfth and thirteenth it was one entity above; clause 3 alone decides what a gap means, and this
one decides nothing.

**One observation the note did not list, read off two cells of the block.** The header's eligible
count and the shipped row's `in pool` at the lowest floor differ by four. By ADR 65's definition
of `in pool`, four eligible entities are in no pool at any floor: the sweep from the rest of the
known list does not reach them. That is stated because the two cells are quoted above and it is
what they mean; it decides nothing, explains nothing, and names nothing.

**The first of the note's sentences applies.** The shipped rate stayed within a few points of the
thirteenth's — within a quarter of a point — so, in the note's words, the run and the session
together did not move the reading. The note's sentence for `in pool` rising while `hits` held does
not apply: `in pool` did not rise beyond the population's own growth.

### What this does and does not establish

- **It does not establish that the shipped setting is the best one.** It establishes that on this
  reading, by this rule, nothing displaced it, on a population the session enlarged from outside
  the concert list. ADR 65's first consequence is the governing one: no row of that table means
  anything on its own.
- **It does not establish anything about `adamic-adar`.** Its rate moved from one entity above the
  shipped scorer's to a few below on a changed population and redrawn folds; the rule's margin
  exists so that a few entities decide nothing. Whether that is a trend is a question for a later
  reading's note.
- **It does not establish anything about the four unreached entities.** Which they are, why the
  sweep does not reach them, and whether a wider hop limit or a second-hop run would, are the
  census's questions and a later note's, not this reading's.
- **The negatives condition was dropped rather than satisfied**, so nothing here is a finding about
  the negatives column; the cells are quoted above, on the entity-fold scale, and decided nothing.
