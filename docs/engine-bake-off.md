...
**This page is a finished experiment, kept because a decision rests on it.** Before segue committed
to a graph database, one domain model and one port were implemented twice — once on Apache
TinkerPop / Gremlin, once on Apache Jena's RDF and SPARQL — and four queries were run against both.
Gremlin won. The decision itself is
[ADR 18, on choosing Gremlin as the graph engine](adr/0018-graph-engine-gremlin.md); this page is
the evidence behind it.

Nothing here is needed to *use* segue — start at the [user guide](user-guide.md) for that. Read
this if you want to know why the storage layer looks the way it does, or if you are considering
revisiting the choice.

Both adapters are still in the tree and both still pass. `TinkerGraphStore` is the one in service;
`JenaGraphStore` is kept as a working reference implementation, and `GraphStoreContract` runs the
comparison against both on every build, so this is a merge gate rather than a program someone
remembers to run. There is no infrastructure to stand up: TinkerGraph and Jena's TxnMem dataset are
both in-process.
