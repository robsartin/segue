...
/**
 * ADR 31, as amended by issue #52: rank paths by specificity first, then by weakest confidence.
 *
 ...
 *       touches — a Walk of Fame star, a hall of fame — is perfectly true and explains nothing.
 * </ul>
 *
...
public final class PathRanking {
   ...
   */
  public static List<PathResult> rank(List<PathResult> paths, ToIntFunction<String> degreeByQid) {
    return paths.stream().sorted(mostExplanatoryFirst(degreeByQid)).limit(MAX_PATHS).toList();
  }

  private static Comparator<PathResult> mostExplanatoryFirst(ToIntFunction<String> degreeByQid) {
    return Comparator.comparing(PathRanking::restsOnAModelGuess)
        .thenComparingInt(path -> hubIntermediates(path, degreeByQid))
   ...
   */
  private static int hubIntermediates(PathResult path, ToIntFunction<String> degreeByQid) {
    List<Hop> hops = path.hops();
    int hubs = 0;
    for (int i = 0; i + 1 < hops.size(); i++) {
      NodeRecord intermediate = hops.get(i).to();
      if (intermediate.kind() == NodeKind.CONCEPT
          && degreeByQid.applyAsInt(intermediate.qid()) >= HUB_DEGREE) {
        hubs++;
      }
    }
    return hubs;
