...
/**
 * One step of an explanation. {@code traversedBackwards} records that the walk went against the
 * edge's stored direction, so rendering can say "was directed by" rather than "directed".
 */
public record Hop(NodeRecord from, EdgeRecord edge, NodeRecord to, boolean traversedBackwards) {

  public String describe() {
    String verb =
        traversedBackwards ? "<-[" + edge.typeCode() + "]-" : "-[" + edge.typeCode() + "]->";
