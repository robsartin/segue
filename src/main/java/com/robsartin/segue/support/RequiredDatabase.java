 ...
 * means resolving it. The first cut of #179 did that with a private copy of the env-or-home rule
 * in each tool, which was a third and fourth copy of the one rule {@link DefaultDatabase} exists
 * to hold. The copies slipped past the design because the fence names {@code DefaultDatabase}
 * literally, and re-implementing a rule never touches the class that owns it. So the resolution
 * happens here, once, by calling {@link DefaultDatabase#resolve} - and the two tools depend on
 * this class instead, not on {@code DefaultDatabase}, which leaves the intended ArchUnit fence
...
public final class RequiredDatabase {
   ...
   *
   * <p>The path is in the sentence so the owner's next command is a copy-paste. {@code SEGUE_DB}
   * is quoted back when it is set - it is genuinely the database the tool would have used - and is
   * still refused, because an agent's shell is initialised from the owner's profile and inherits
   * it. A variable cannot tell the owner apart from an agent running as the owner; a flag typed
   * per invocation can.
   *
