 ...
 * picture in a heavier syntax. Nodes carry {@code kind} and {@code label}; edges carry {@code
 * typeCode}, {@code confidence} and {@code sourceId}. Confidence is the one that earns its place:
 * ADR 31 ranks a route by its weakest hop and demotes hub intermediates, so "show me every edge
 * under 1.00" and "show me what a model proposed" are exactly the questions a person opens one of
 * these files to ask.
 *
...
public final class GraphMlWriter implements ViewWriter {
  ...
  private static final List<Key> NODE_KEYS =
      List.of(new Key("kind", "node", "string"), new Key("label", "node", "string"));
