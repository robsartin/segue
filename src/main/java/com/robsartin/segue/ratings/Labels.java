  ...
  static Map<String, String> forQids(AssertionLog log, Set<String> qids) {
    ...
    for (int i = 0; i < logged.size(); i++) {
      ...
      if (assertion instanceof SameAs merge && merges.stands(merge)) {
        // A merge a later one corrected lends a label only where an edge the fold keeps still
        // names its canonical id (#221, widened in a later round of the same issue): stands() is
        // then true because the node survives on that edge's account, not because this merge
        // still resolves anything. Where stands() is false - no kept edge either - a label here
